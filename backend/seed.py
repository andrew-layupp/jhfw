"""
seed.py
On first run (empty DB), backfills 6 months of weekly chaos scores
using real yfinance market data + interpolated geo/trade/climate values
based on known events in the Oct 2025 – Apr 2026 period.
"""
import logging
from datetime import datetime, timedelta
import pandas as pd

from database import count_snapshots, insert_snapshot
...
# Format: (date_str, geo_adj, trade_adj)
# These adjustments are layered on top of the VIX-correlated baseline.
EVENT_ANCHORS = [
    # (date,          geo_delta, trade_delta)
...
def _base_geo(vix: float) -> float:
    """Geo baseline: correlated with VIX + structural floor ~55."""
    return clamp(55 + (vix - 20) * 0.8)


def _base_trade(vix: float) -> float:
    """Trade baseline: correlated with VIX + structural floor ~50."""
    return clamp(50 + (vix - 20) * 0.7)


def _event_delta(date: datetime, col: str) -> float:
    """Sum up event adjustments active within ±14 days of each anchor."""
    total = 0.0
...
async def seed_if_empty():
    ...
    if not frames.get("vix") is not None and "vix" not in frames:
    ...
    def weekly(s): return s.resample("W-MON").last().dropna()
    ...
    for ts in vix_w.index:
        ...
        try:
            ...
            climate = 65.0   # static for historical seed
            living  = clamp(55 + (date - start).days / 185 * 15)  # gradual rise
            ...
            await insert_snapshot({
                ...
                "overall": overall,
                "scores": {
                    "geo": round(geo, 1), "markets": round(markets, 1),
                    "energy": round(energy, 1), "trade": round(trade, 1),
                    "climate": round(climate, 1), "living": round(living, 1),
                },
                "raw": {
                    "vix": round(vix, 2), "oil": round(oil, 2),
                    "gold": round(gold, 2), "spy": round(spy, 2),
